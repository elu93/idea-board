...
class IdeaPage extends Component {
    ...
    deleteIdea = async(idea) => {
        try {
            ...
            const indexToDelete = this
                .state
                .ideas
                .indexOf(idea) // Determine where in our ideas array it lived
            const newIdeas = [...this.state.ideas] // copy the old ideas list into a new one
            newIdeas.splice(indexToDelete, 1) // remove the idea we deleted from this new array

            // update the state with our new ideas list, so the deleted idea will no longer
    ...
    // We need to pass in multiple arguments here.  The first is the object of the
    // specific idea that is being changed. And the event object is the special
    // event listener object that has information about the value and name
    handleChange = (idea, event) => {
        ...
        // Here we are using bracket syntax instead of dot-notation to transform a
        // specific property More info on bracket syntax here
        // https://medium.com/@prufrock123/js-dot-notation-vs-bracket-notation-797c4e34f0
        // 1d
    ...
    updateIdea = async(idea) => {
        try {
            ...
            // We don't need to do anything else here, because the state was already updated
            // by our handleChange method
